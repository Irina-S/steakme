...
//   }catch(e){}
  ...
  try{
    $('select').niceSelect();
    $('.title-block__filter select').change(function(){
  ...
  if($('.order-page').length > 0){
    const promoWrapper = $('.order-item-group__wrapper');

    $('.order-promo-item__btn').click(function () {
      $(promoWrapper).addClass('order-item-group__wrapper--opened');
    });

    $('.order-promo-item__close-btn').click(function () {
      $(promoWrapper).removeClass('order-item-group__wrapper--opened');
    });

/*
    $('.order-promo-item__form').submit(function () {
      const promoValue = $('.order-promo-field', $(this)).val();
      //место отправки запроса к бэку
      if(1){
        const promoDiscount = 300;
        $('.order-promo-item--promo-output').addClass('order-promo-item--applied');
        $('.order-item-group__wrapper').removeClass('order-item-group__wrapper--opened');
        $('.order-promo-item__price').html('- ' + promoDiscount + ' руб.')
      } else{
        $('.order-promo-item--promo-input').addClass('order-summary__item--error');
      }
      return false;
    });
*/
    ...
    function isStep2Success(){
        if($('input[name=shk_delivery]:checked').val() === 'Доставка по адресу'){
        $('.bymslf-tab').addClass('hidden');
        $('.delivery-tab').removeClass('hidden');
        checkDeliveryAddr();
      } else if($('input[name=shk_delivery]:checked').val() === 'Самовывоз'){
        $('.delivery-tab').addClass('hidden');
        $('.bymslf-tab').removeClass('hidden');
        $('#order-step-2').addClass('success');
        $('#order-step-2 .error-field').css('display', 'none');
      }
    }
    
    // $('input[name=shk_delivery]:first-child').first().prop("checked", true);
    isStep2Success();

    $('#order-step-2 input.required-group').change(function () {
      checkDeliveryAddr();
    });

    function checkDeliveryAddr(){
    ...
    function isStep3Success(){
      if($('input[name=time]:checked').val() === 'Выбрать время'){
        $('.choose-time-tab').removeClass('hidden');
      } else{
        $('.choose-time-tab').addClass('hidden');
      }
    ...
    function isStep1Success(){
      ...
      if(name.val().length !== 0 && phoneIsValid(phone.val()) && !phone.hasClass('has-error')){
    ...
    $('#order-step-4 input').change(function () {
      isStep4Success();
    });
    
    function isStep4Success(){
        if($('.curr-pay input[name=pay-type]:checked').length > 0){
            $('#order-step-4').addClass('success');
        } else{
            $('#order-step-4').removeClass('success');
        }
    }
  ...
  // СВЯЗЬ РАДИО-КНОПОК ДОСТАВКИ И ОПЛАТЫ
    ...
    function setText(){
        const deliveryType = $('#shopOrderForm input[name="shk_delivery"]:checked').attr('id');
        if (deliveryType=='delivery'){
            $('label[for="time-1"] span:nth-child(2)').text('Доставим в течение часа').css('display', 'block');
            $('label[for="time-2"] span:nth-child(2)').text('Доставим в выбранное время').css('display', 'block');
        }
        else if (deliveryType=='by-myself'){
            $('label[for="time-1"] span:nth-child(2)').text('Приготовим в течение часа').css('display', 'block');
            $('label[for="time-2"] span:nth-child(2)').text('Приготовим ко времени').css('display', 'block');
        }
    }
